...
def get_report_summary(data):
	lead_types = ["Staffing", "ERP", "CRM", "CMS", "AI", "Custom Development", "Existing Client", "Consultant", "Channel Partner"]
	...
	# Add the Total Leads card last
	summary.append({
		"value": total_count,
		"indicator": "Blue" if total_count > 0 else "Grey",
		"label": _("Total Leads"),
		"datatype": "Int",
	})
...
def get_data(filters):
	...
	if filters.get("from_date") and filters.get("to_date"):
		...
		query = query.where(
			Date(lead.creation) >= filters.get("from_date")
		)
	elif filters.get("to_date"):
		query = query.where(
			Date(lead.creation) <= filters.get("to_date")
		)
